#' Zooplankton dynamics (Biomass)
#' 
#' This function allows you to express the zooplankton dynamics using GOCI
...
pzoo <- function(x) {
  ...
  for(i in 1:length(x1)){
    ...
    p.plot = plot(x2*0.51/mean(x2), type = 'l', ylim = c(0,1), xlab = 'Days', ylab = "Biomass (%)",axes = F, col = 'red')
    +axis(1)
    +axis(2)
    box()
    print(p.plot)
    return(x2*0.51/mean(x2))
  }
}